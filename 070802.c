...
void display_stone(int matrix[][20]); // 바둑알 놓기 함수 정의 
...
{
    int i, j; //변수 i, j 정의 
    unsigned char a=0xa6; //unsigned char 변수 a 정의 
    unsigned char b[12];  //unsigned char 변수 b 정의 
    for(i=1;i<12;i++)
	b[i]=0xa0+i; //변수 i가 1부터 12미만 까지 b[i]의 값에 1씩 증가를 반복 
    printf("%c%c",a, b[3]); //화면에 변수 a와 b[3]을 출력 
    for(i=0;i<c-1;i++)
		printf("%c%c", a, b[8]); //변수 i가 1부터 c-1미만 까지 화면에 변수 a와 b[8]을 반복 출력 
    printf("%c%c", a, b[4]); //화면에 변수 a와 b[4]를 출력 
    printf("\n"); //줄 바꿈 
    for(i=0;i<r-1;i++) //변수 i가 0부터 r-1미만 까지 아래 실행들을 반복 
    {
		printf("%c%c", a, b[7]); //화면에 변수 a와 b[7]을 출력 
		for(j=0;j<c-1;j++) 
	    	printf("%c%c", a, b[11]); //변수 j가 0부터 c-1미만까지 화면에 변수 a와 b[11]을 반복 출력 
		printf("%c%c",a, b[9]); //화면에 변수 a와 b[9]을 출력 
		printf("\n"); //줄 바꿈 
    }
    printf("%c%c", a, b[6]); //화면에 변수 a와 b[6]을 출력 
    for(i=0;i<c-1;i++)
		printf("%c%c", a, b[10]); //변수 i가 0부터 c-1미만까지 화면에 변수 a와 b[10]을 반복 출력 
...
{
    int x=1, y=1, matrix[20][20]={0}; // int 변수 x, y, matrix 생성 
...
void display_stone(int matrix[][20]) // 바둑알 놓기 함수 생성 
{
	int x, y; //int 변수 x, y 생성 
	for(x=1;x<20;x++) //변수 x가 1부터 20미만까지 아래 실행들을 반복 
		for(y=1;y<20;y++) // 변수 y가 1부터 20미만까지 아래 실행들을 반복 
		{
			if (matrix[x][y]==1) //matrix[x][y]가 1일때 코드를 실행 
			{
				gotoxy(x*2-1, y);
				printf("●");
			}
...
void move_arrow_key(char key, int *x1, int *y1, int x_b, int y_b) // 방향키 함수 생성 
{
	switch(key)
	{
